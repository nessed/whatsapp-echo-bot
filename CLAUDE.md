...
- [x] `Allowed Sender` IF node (capital S — that's the real node name) — **built and verified.** Sits after `Respond OK (msg)`, not directly after `Log to Supabase`. Compares `={{ $('Edit Fields').item.json.from_number }}` (string equals) against Fixed-mode `923000413777`. **Not** `$json.from_number` — see the step-9 `$json` deviation below.
...
- [ ] `text message` FALSE branch (fixed "text messages only" reply + outbound log) — not built.
...
- [ ] Outbound logging to `messages` (`direction: 'out'`) — **not built.** Until this exists `Load history` only ever returns the user's own messages, so DeepSeek has no memory of its own replies across turns.
- [ ] `Complete run` PATCH of `assistant_runs` (status → `completed`, `outbound_wa_message_id`, usage tokens) — not built.
- [ ] Failure path (mark the run `failed` when DeepSeek or the Meta send errors) — not built.
...
- [~] 9. Build the manual DeepSeek branch and outbound logging in n8n — **reply path done and proven live; bookkeeping still open.** The full inbound → DeepSeek → outbound chain works end to end: executions 37 and 40 were real messages from the handset that got real DeepSeek answers delivered back, with Meta returning valid wamids. Still missing: outbound logging, run completion, the non-text branch, and the failure path (see the sub-checklist above).

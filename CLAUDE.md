...
- [ ] `Allowed sender` IF node — **status unconfirmed.** Instructions were given (after `Log to Supabase`, compare `={{ $json.from_number }}` equals `923000413777`) but session ended before confirming it was actually built. **Check this first** before assuming it's there.
- [ ] `Text message` IF node — instructions given (on `Allowed sender`'s true branch, compare `={{ $json.message_type }}` equals `text`), not yet confirmed built. Its two branches (fixed non-text reply vs. the real DeepSeek pipeline) are not yet built at all.
- [ ] Everything from `context/07` §4 onward (claim assistant run, history fetch, DeepSeek call, send+log, failure path) — not started.
